...
inherited by the main export command. Supports both Excel and CSV formats.
"""
import pandas as pd
import os
...
class ExportMethodsMixin:
    ...
    Supports both Excel (single file with worksheets) and CSV (separate files) formats.
    """
    ...
    def export_vinyl_data_csv(self, base_name, export_dir):
        """Export vinyl records data to CSV file."""
        self.stdout.write('Exporting vinyl records data to CSV...')
        ...
        df = pd.DataFrame(data)
        csv_path = os.path.join(export_dir, f'{base_name}_vinyl.csv')
        df.to_csv(csv_path, index=False)
        self.stdout.write(f'  - Exported {len(data)} vinyl records to {csv_path}')
    ...
    def export_orders_data_csv(self, base_name, export_dir):
        """Export orders data to CSV file."""
        self.stdout.write('Exporting orders data to CSV...')
        ...
        # Create DataFrame and export to CSV
        df = pd.DataFrame(data)
        csv_path = os.path.join(export_dir, f'{base_name}_orders.csv')
        df.to_csv(csv_path, index=False)
        self.stdout.write(f'  - Exported {len(data)} order records to {csv_path}')
    ...
    def export_cart_data_csv(self, base_name, export_dir):
        """Export cart data to CSV file."""
        self.stdout.write('Exporting cart data to CSV...')
        ...
        # Create DataFrame and export to CSV
        df = pd.DataFrame(data)
        csv_path = os.path.join(export_dir, f'{base_name}_cart.csv')
        df.to_csv(csv_path, index=False)
        self.stdout.write(f'  - Exported {len(data)} cart records to {csv_path}')
    ...
    def export_reviews_data_csv(self, base_name, export_dir):
        """Export reviews data to CSV file."""
        self.stdout.write('Exporting reviews data to CSV...')
        ...
        # Create DataFrame and export to CSV
        df = pd.DataFrame(data)
        csv_path = os.path.join(export_dir, f'{base_name}_reviews.csv')
        df.to_csv(csv_path, index=False)
        self.stdout.write(f'  - Exported {len(data)} review records to {csv_path}')
    ...
    def export_wishlist_data_csv(self, base_name, export_dir):
        """Export wishlist data to CSV file."""
        self.stdout.write('Exporting wishlist data to CSV...')
        ...
        # Create DataFrame and export to CSV
        df = pd.DataFrame(data)
        csv_path = os.path.join(export_dir, f'{base_name}_wishlist.csv')
        df.to_csv(csv_path, index=False)
        self.stdout.write(f'  - Exported {len(data)} wishlist records to {csv_path}') 